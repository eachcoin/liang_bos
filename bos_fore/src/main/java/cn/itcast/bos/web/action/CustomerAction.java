...
//        }
    ...
    @Action(value = "customer_login", results = {@Result(name = "login", location = "login.html", type = "redirect"),
            @Result(location = "index.html#/myhome", type = "redirect")})
    public String login() {
        Customer customer = WebClient.create(Constants.CRM_MANAGEMENT_URL + "/services/customerService/customer/login?telephone=" + model.getTelephone() + "&&password=" + model.getPassword()).get(Customer.class);
        if (customer == null) {
            // 登录失败
            return LOGIN;
        } else {
